import { User, LayoutGrid } from "lucide-react";
import { CampaignLink } from "../ui/campaign-link";
import { CampaignCombobox } from "../CampaignSelector";
import { cn } from "../../lib/utils";
import { useLocation } from "react-router-dom";

export const Sidebar = () => {
  const location = useLocation();

  const buildClassName = (path: string) => {
    return cn(
      "border-b border-[#F1F1F4] rounded-none flex flex-row items-center gap-2 flex-1 justify-center md:justify-start py-4",
      {
        "text-[#2E9858] border-b-2 border-[#2E9858] md:border-b-1 md:border-[#F1F1F4]":
          location.pathname === path,
      }
    );
  };

  return (
    <div className="flex flex-col gap-4">
      <CampaignCombobox placeholder="Select campaign..." className="w-full" />

      <div className="flex flex-row md:flex-col">
        <CampaignLink to="/" className={buildClassName("/")}>
          <LayoutGrid className="h-4 w-4" />
          <span>Dashboard</span>
        </CampaignLink>

        <CampaignLink to="/employees" className={buildClassName("/employees")}>
          <User className="h-4 w-4" />
          <span>Empleados</span>
        </CampaignLink>
      </div>
    </div>
  );
};
